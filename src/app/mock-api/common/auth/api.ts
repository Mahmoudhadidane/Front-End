...
import { user as userData,admin } from 'app/mock-api/common/user/data';
...
export class AuthMockApi
{
    ...
    registerHandlers(): void
    {
                ...
                if ( request.body.email === ('hughes.brian@company.com' ) && request.body.password === 'admin' )
                {
                    return [
                        200,
                        {
                            ...
                            accessToken: this._generateJWTToken(this._user.role),
                            role:this._user.role,
                            tokenType  : 'bearer'
                        }
                    ];
                }
                if ( request.body.email === ('admin@admin' ) && request.body.password === 'admin' )
                    {
                        return [
                            200,
                            {
                                user       : cloneDeep(this._admin),
                                role:this._admin.role,
                                accessToken: this._generateJWTToken(this._admin.role),
                                tokenType  : 'bearer'
                            }
                        ];
                    }
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                const role = this._decodeToken(accessToken).role
                // Verify the token
                if ( this._verifyJWTToken(accessToken) )
                {
                    return [
                        200,
                        {
                            user       : cloneDeep(this._user),
                            role:role,
                            accessToken: this._generateJWTToken(role),
                            tokenType  : 'bearer'
                        }
                    ];
                }
        ...
        this._fuseMockApiService
            ...
            this._fuseMockApiService
            .onPost('api/auth/get-role')
            .reply(({request}) => {
                const accessToken = request.body.token;
                const role = this._getRoleFromToken(accessToken)
                return [
                    200,
                    {
                        role:role
     ...
     * @private
     */
    private _generateJWTToken(role:string): string
    {
        // Define token header
        const header = {
            alg: 'HS256',
     ...
     * @returns 
     */
    private _getRoleFromToken(token:string):string{
 ...
 * @private
 */
public _decodeToken(token: string): any {
    // Split the token into parts
    const parts = token.split('.');
    if (parts.length !== 3) {
    ...
    // Decode the payload
    const payload = parts[1];
    const decodedPayload = Base64.parse(payload).toString(Utf8);
